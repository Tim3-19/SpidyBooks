import React from 'react'
import { Link } from 'react-router-dom'

const Hero = () => {
  return (
    <div className='h-[75vh] flex flex-col md:flex-row items-center justify-center'>
        <div className='w-full mb-12 md:mb-0 lg:w-3/6 flex flex-col items-center lg:items-start justify-center'>
        <h1 className="text-5xl md:text-6xl font-bold leading-tight text-yellow-100">
            Discover Your next Great Read</h1>
            <p className='mt-4 text-xl text-zinc-300'>Unravel the passion of reading</p>
            <div className='mt-8'>
                <Link to ="/all-books"
                 className='text-yellow-100 text-xl lg:text-2xl font-semibold border border-yellow-100 px-10 py-3 hover:bg-zinc-800 rounded-full'>
                    Discover More
                 </Link>
            </div>
            </div>
        <div className='w-full lg:w-3/6 h-auto lg:h-full flex items-center justify-center '>
          <img src="./background.jpg" alt="hero" className="w-full object-cover mt-4 rounded-3xl"/>
        
        </div>
    </div>
  )
}

export default Hero