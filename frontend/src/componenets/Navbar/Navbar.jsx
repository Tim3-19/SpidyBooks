...
const Navbar = () => {
  ...
  const visibleLinks = isLoggedIn
    ? links
    : links.filter((l) => !["Cart", "Profile"].includes(l.title));
  ...
  return (
    <>
      ...
      */}
      <nav className="fixed top-0 left-0 w-full z-50 bg-zinc-900/80 backdrop-blur-md border-b border-zinc-700/50 shadow-lg transition-all duration-300">
          ...
          <div className="hidden md:flex items-center gap-8">
            ...
            {!isLoggedIn && (
              <div className="flex gap-4">
                ...
                <Link
                  to="/signUp"
                  className="px-5 py-2 text-sm font-semibold bg-blue-600 text-white rounded-full hover:bg-blue-500 shadow-lg shadow-blue-500/20 hover:shadow-blue-500/40 transition-all duration-300"
                >
                  Sign Up
                </Link>
              </div>
      ...
      {/* Mobile Menu Overlay */}
      {mobileNav && (
        <div className="fixed inset-0 bg-zinc-900 z-50 flex flex-col items-center justify-center gap-8 animate-fadeIn">
            {/* Close Button */}
            <button 
